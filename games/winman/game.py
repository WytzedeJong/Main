...
class WinMan(Scene):
    def __init__(self, manager):
        ...
        self.boss_quotes_phase2 = ["...bad time.", "It's over buddy.", "OVERCLOCK MODE", "REBOOTING...",
                                   "ACCESS DENIED"]
        ...
        self.tutorial_texts = [
            ...
            "Press L to give a compliment in this demo.",
    ...
    def update(self, dt):
        ...
        if self.state == "TUTORIAL":
            ...
            if self.tutorial_step == 1:
                if self.manager.input_handler.is_pressed("UP"): self.soul_pos[1] -= self.soul_speed
                if self.manager.input_handler.is_pressed("DOWN"): self.soul_pos[1] += self.soul_speed
                if self.manager.input_handler.is_pressed("LEFT"): self.soul_pos[0] -= self.soul_speed
                if self.manager.input_handler.is_pressed("RIGHT"): self.soul_pos[0] += self.soul_speed
        ...
        if self.state == "FIGHT_TIMING":
            ...
            if self.phase == 1:
                ...
                if self.attack_type == "bouncing_balls" and self.attack_timer % ball_interval == 0 and len(
                        self.bullets) < max_balls:
                ...
                bone_interval = int(23 - 10 * p)
                bone_speed = 2.8 + 0.8 * p
                ...
                p2_ball_interval = int(28 - 8 * p)
                p2_ball_max = int(5 + 3 * p)
                p2_ball_spd = 3.4 + 0.8 * p
                if self.attack_type == "bouncing_balls_hard" and self.attack_timer % p2_ball_interval == 0 and len(
                        self.bullets) < p2_ball_max:
                ...
                if self.attack_type == "circle_strike" and self.attack_timer % 70 == 0:
                    ...
                    for _ in range(ring_count):
                        ...
                        gap_indices = {gap_start, (gap_start + 1) % bullet_count}
                ...
                if self.attack_type == "cross_laser" and self.attack_timer % 40 == 0:
                    ...
                    y_top = min(y_top, self.box_rect.bottom - beam_height)
                    self.bullets.append(
                        {"type": "laser", "x": target_x, "timer": 20, "telegraph": 18, "telegraph_warn": 12})
                    self.bullets.append(
                        {"type": "h_laser", "y": y_top, "height": beam_height, "timer": 16, "telegraph": 16,
                         "telegraph_warn": 10})
            ...
            for i in range(len(self.bullets) - 1, -1, -1):
                ...
                if b["type"] == "laser":
                    if b.get("telegraph_warn", 0) > 0:
                        ...
                        if player_rect.colliderect(
                                pygame.Rect(b["x"] - 6, self.box_rect.top, 12, self.box_rect.height)):
                ...
                if player_rect.colliderect(bullet_hitbox):
                    ...
                    if b["type"] not in ("bone", "bounce"):
                        del self.bullets[i]
                elif b["type"] not in ("bounce",) and not self.box_rect.inflate(60, 60).collidepoint(b["pos"][0],
                                                                                                     b["pos"][1]):
                    del self.bullets[i]
    ...
    def _draw_scene(self, surface):
        ...
        if self.state in ["MENU", "MENU_EXIT_POPUP", "MESSAGE", "MONOLOGUE"]:
            ...
            if self.state == "MONOLOGUE":
                prompt = self.font_ui.render("[L]", True, self.YELLOW)
            ...
            if self.state == "MENU_EXIT_POPUP":
                ...
                hint = self.font_ui.render("b", True, self.YELLOW)
