...
class MonkeyStacker(Scene):
            ...
            for mx in range(size):
                for my in range(size):
                    ...
                    if dx * dx + dy * dy > (size // 2) ** 2:
                        self.monkey_surf.set_at((mx, my), (0, 0, 0, 0))
        except Exception:
            size = self.tile * 2 + 4
            self.monkey_surf = pygame.Surface((size, size), pygame.SRCALPHA)
    ...
    def _build_background(self):
        ...
        self._draw_foliage_layer(self._bg, (20, 70, 25),  y_base=BASE_HEIGHT,      count=14, w_range=(30, 60), h_range=(50, 100))
        self._draw_foliage_layer(self._bg, (25, 90, 30),  y_base=BASE_HEIGHT - 10, count=10, w_range=(20, 45), h_range=(35,  70))
        self._draw_foliage_layer(self._bg, (35, 110, 40), y_base=BASE_HEIGHT,       count=8,  w_range=(15, 35), h_range=(20,  45))
    ...
    def _load_highscore(self):
        try:
            with open(self._scores_path(), "r", encoding="utf-8") as f:
                self.highscore = int(json.load(f).get("best", 0))
        except Exception:
            self.highscore = 0
    ...
    def _save_highscore(self):
        try:
            with open(self._scores_path(), "w", encoding="utf-8") as f:
                json.dump({"best": (self.highscore // 10) * 10}, f)
        except Exception:
            pass
    ...
    def _draw_monkey_and_block(self, surface):
        ...
        for seg in range(segments):
            ...
            x0 = int(center_x + sway * t0); y0 = int(t0 * rope_end_y)
            x1 = int(center_x + sway * t1); y1 = int(t1 * rope_end_y)
        ...
        mx   = center_x - mw // 2
        my   = rope_end_y - mh // 4
        pygame.draw.circle(surface, (30, 15, 5), (center_x, my + mh // 2), mw // 2 + 3)
    ...
    def _draw_start_screen(self, surface):
        ...
        self._draw_stylish_button(surface, "PRESS B TO START",  cy + 48)
    ...
    def _draw_game_over(self, surface):
        ...
        self._draw_stylish_button(surface, "PRESS B TO RESTART", cy + 42, color=(170, 30, 30))
        self._draw_stylish_button(surface, "HOME", cy + 78, color=(60, 60, 160))
