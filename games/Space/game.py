...
class SpaceGame(Scene):
    def __init__(self, manager):
        ...
        self.asset_dir = os.path.dirname(__file__)
        ...
        self.player_x = 48.0
        self.player_y = BASE_HEIGHT / 2 - self.player_size[1] / 2
    ...
    def get_random_upgrade_y(self, upgrade_height):
        min_y = 18
        max_y = BASE_HEIGHT - upgrade_height - 18
    ...
    def spawn_round(self, round_number):
        ...
        spacing = BASE_HEIGHT / (enemy_count + 1)
        ...
        for index in range(enemy_count):
            ...
            enemy = Enemy(
                x=x,
                y=max(18, min(BASE_HEIGHT - height - 18, y)),
                width=width,
                height=height,
    ...
    def update_player(self, dt):
        ...
        self.player_y = max(14, min(BASE_HEIGHT - self.player_size[1] - 14, self.player_y))
    ...
    def spawn_player_bullet(self):
        bullet = Bullet(
            ...
            y=self.player_y + self.player_size[1] / 2,
            vx=self.player_bullet_speed,
            vy=0.0,
    ...
    def update_enemies(self, dt):
        for enemy in self.enemies:
            ...
            min_y = 10
            max_y = BASE_HEIGHT - enemy.height - 10
    ...
    def update_upgrade(self, dt):
        ...
        if self.upgrade_drop.rect.colliderect(player_rect):
            self.apply_upgrade(self.upgrade_drop.effect)
    ...
    def check_round_clear(self):
        ...
        if cleared_round % 2 == 0:
            if not self.upgrade_pool:
                self.reset_upgrade_pool()

            upgrade = self.upgrade_pool.pop()
    ...
    def apply_upgrade(self, effect):
        if effect == "fire_rate":
            ...
            self.player_health = min(self.max_health, self.player_health + 25)
    ...
    def draw_background(self, surface):
        if self.current_background:
            ...
            surface.fill((7, 10, 26))

        player_zone = pygame.Rect(0, 0, int(BASE_WIDTH * 0.45), BASE_HEIGHT)
        enemy_zone = pygame.Rect(int(BASE_WIDTH * 0.58), 0, BASE_WIDTH, BASE_HEIGHT)
        shade = pygame.Surface((BASE_WIDTH, BASE_HEIGHT), pygame.SRCALPHA)
        pygame.draw.rect(shade, (8, 14, 28, 105), player_zone)
        pygame.draw.rect(shade, (24, 10, 20, 95), enemy_zone)
        surface.blit(shade, (0, 0))
        pygame.draw.line(surface, (70, 90, 150), (BASE_WIDTH // 2, 8), (BASE_WIDTH // 2, BASE_HEIGHT - 8), 1)
    ...
    def draw_bullets(self, surface):
        for bullet in self.bullets:
            color = (110, 235, 255) if bullet.owner == "player" else (255, 110, 110)
    ...
    def draw_hud(self, surface):
        hud_rect = pygame.Rect(BASE_WIDTH - 112, 8, 98, 46)
        pygame.draw.rect(surface, (8, 12, 20), hud_rect, border_radius=10)
        pygame.draw.rect(surface, (200, 220, 255), hud_rect, 2, border_radius=10)

        health_bar_bg = pygame.Rect(hud_rect.x + 8, hud_rect.y + 8, 82, 8)
        health_fill = int(health_bar_bg.width * self.player_health / self.max_health)
        ...
        hp_text = self.small_font.render(f"HP {int(self.player_health)}/{int(self.max_health)}", True, (255, 255, 255))
        score_text = self.small_font.render(f"S {self.score}", True, (255, 255, 255))
        round_text = self.small_font.render(f"R {self.round_number}", True, (255, 255, 255))
        surface.blit(hp_text, (hud_rect.x + 8, hud_rect.y + 19))
        surface.blit(score_text, (hud_rect.x + 8, hud_rect.y + 31))
        surface.blit(round_text, (hud_rect.x + 50, hud_rect.y + 31))

    def draw_round_banner(self, surface):
        label = self.big_font.render(f"Round {self.round_number}", True, (255, 240, 180))
        rect = label.get_rect(center=(BASE_WIDTH // 2, 24))
        panel = rect.inflate(20, 10)
        pygame.draw.rect(surface, (16, 18, 32), panel, border_radius=8)
        pygame.draw.rect(surface, (255, 240, 180), panel, 2, border_radius=8)
        ...
        sections = [
            ...
            (
                ...
                [
                    ...
                    "Na elke 2 rondes dropt er een stat upgrade op een willekeurige hoogte.",
                    "De 8 upgrades blijven in een shuffle-cyclus terugkomen.",
